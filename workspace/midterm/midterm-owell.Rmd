...
Instructor Protocol: Since in class, the instructor mentioned refusing to work with bad data, data collection must be important. Since we've had to extract data and make it readable in assignments, data organization/cleaning is important. Since we have to think about what we're doing and why we're doing it when we are analyzing and interpreting the data, analysis and interpretation are important too. Because "one graphic" is a theme of the class, presentation is also important. 
...
To learn something, you definitely have to do it. You could read a book on riding horses, watch people ride horses all day, and still be absolutely horrible at riding when you try it yourself. The same thing applies to math. You can read the textbook, listen to the lecture, watch people solve problems, but until you try it yourself, you don't fully understand how to do it.  
...
Also, according to psychology, there is a such thing as latent learning, where it seems like no learning is occuring but then all of a sudden, it happens. There was a study with rats in mazes and the rats who were the slowest originally became the fastest once they started finishing the maze. Perhaps they explored all of the incorrect ways of doing things so they had the knowledge and confidence to go the correct way every time because they knew where the other paths led. 
...
If you're your own boss, then you would have to be intrinsically motivated to get anything done because there wouldn't be external rewards/punishments. Without external pressure and with no intrinsic motivation, it would be easy to put off tasks. 

People with an E-5 personality would likely be instrinsically motivated to talk to people. 

People with a D-5 personality would likely be instrinsically motivated to think things through thoroughly.
...
People with a D-1 personality would likely be intrinsically motivated to do things creatively.
...
Some key aspects about standardized percentiles are that scores are compared to a reference group. Percentiles on standardized tests can change across years. We can compare percentiles across samples when the means and standard deviations are the same or when we standardize the samples so they then have the same mean and standard deviation (like z-scores). A percentile shows more about how well a person did in comparison to others than an average score. An average score doesn't really show outliers (although outliers can affect it), but percentiles show the outliers. 
...
geologyRank isn't square so the code doesn't work. I'm either trying to find the eigenRank of the wrong matrix, it's not supposed to be able to work, or I'm making some other error I can't figure out before the midterm is due. Since the code doesn't work, I have no idea which wll ranks the highest or lowest. My guess is that the highest and lowest would be the same if it behaved like the chemistryRanks. 
